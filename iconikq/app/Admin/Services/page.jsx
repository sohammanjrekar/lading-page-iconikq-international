...
const ServicesPage = () => {
  ...
  const handleAddOrUpdateService = async () => {
    ...
    if (result.error) {
      ...
      showMessage(
        "success",
        editId ? "Service updated successfully!" : "Service added successfully!"
      );
      fetchServices();
      clearFields();
    }
  ...
  return (
    <>
      ...
      <div className="mt-8 text-center font-bold text-2xl m-5 text-gray-800 container">
        All Services
      </div>
        ...
        {services.map(
          (serv) =>
            serv && (
              <div
                key={serv.id}
                className="service-card rounded-lg border border-gray-300 p-4"
              >
                <img
                  src={serv.image}
                  alt={serv.title}
                  className="rounded-lg my-5 h-40 object-cover w-auto"
                />
